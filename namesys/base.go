...
type resolver interface {
	...
	secureResolveOnce(ctx context.Context, cw coreiface.ChunkWriter, name string, options *opts.ResolveOpts) (value path.Path, ttl time.Duration, err error)
}
...
func secureResolve(ctx context.Context, cw coreiface.ChunkWriter, r resolver, name string, options *opts.ResolveOpts, prefixes ...string) (path.Path, error) {
	depth := options.Depth
	for {
		p, _, err := r.secureResolveOnce(ctx, cw, name, options)
		if err != nil {
			return "", err
		}
		log.Debugf("resolved %s to %s", name, p.String())

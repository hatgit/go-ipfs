...
func (r *IpnsResolver) SecureResolve(ctx context.Context, cw coreiface.ChunkWriter, name string, options ...opts.ResolveOpt) (path.Path, error) {
	return secureResolve(ctx, cw, r, name, opts.ProcessOpts(options), "/ipns/")
}
...
func (r *IpnsResolver) secureResolveOnce(ctx context.Context, cw coreiface.ChunkWriter, name string, options *opts.ResolveOpts) (path.Path, time.Duration, error) {
	log.Debugf("RoutingResolver resolving %s", name)
	...
	if err := cw.WriteChunk(append([]byte{1}, val...)); err != nil {
		return "", 0, err
	}
